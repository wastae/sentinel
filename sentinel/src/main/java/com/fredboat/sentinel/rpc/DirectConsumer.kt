...
class DirectConsumer(
    ...
    init {
        ...
        socketIOServer.addEventListener("sendSlashMenuCommandRequest-${key.key}", JSONObject::class.java) { client, request, _ ->
            CoroutineScope(Dispatchers.IO).launch {
                message.consume(SocketServer.gson.fromJson(request.toString(), SendSlashMenuCommandRequest::class.java), client)
            }
        }
